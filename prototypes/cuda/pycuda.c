...
__global__ void newCenters(double *data, int *labels, double *centers)
{
    ...
    // find KxD centers
    centers[tid] = l_sum/s_clustern[k];

}

__global__ void reassign(double *data, int *labels, double *centers, int *converged)
{
    ...
    // get KxD squares
        s_squares[tid] = (data[dataid] - centers[tid]) * (data[dataid] - centers[tid]);
        __syncthreads();
